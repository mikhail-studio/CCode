...
M.open = function(target)
    ...
    elseif INFO.listName[blockName][paramsIndex + 1] == 'local' and ALERT then
        BLOCKS.group[8]:setIsLocked(true, 'vertical')
        LOGIC.new('tables', blockIndex, paramsIndex, COPY_TABLE(paramsData), true)
    elseif INFO.listName[blockName][paramsIndex + 1] == 'table' and ALERT then
        BLOCKS.group[8]:setIsLocked(true, 'vertical')
        LOGIC.new('tables', blockIndex, paramsIndex, COPY_TABLE(paramsData))
    elseif INFO.listName[blockName][paramsIndex + 1] == 'fun' and ALERT then
        BLOCKS.group[8]:setIsLocked(true, 'vertical')
