...
M['setTransitionPosNoob'] = function(params)
    ...
    if type == '(select[\'pic\']())' then type = 'GAME_objects'
    ...
    local posX = posX == 'nil' and 'nil' or type == 'GAME_groups' and posX or 'SET_X(' .. posX .. ')'
    local posY = posY == 'nil' and 'nil' or type == 'GAME_groups' and '0 - (' .. posY .. ')' or 'SET_Y(' .. posY .. ')'
    ...
    GAME.lua = GAME.lua .. ' \'' .. type .. '\' == \'GAME_tags\' then pcall(function() local function doTag(tag) for _, child'
...
M['setTransitionSizeNoob'] = function(params)
    local name, type, size = CALC(params[1]), CALC(params[2], 'GAME_objects'), 'nil'
    ...
    GAME.lua = GAME.lua .. ' size = ' .. size .. '}) end if \'' .. type .. '\' == \'GAME_tags\' then pcall(function() local function'
...
M['setTransitionScaleNoob'] = function(params)
    ...
    GAME.lua = GAME.lua .. ' if \'' .. type .. '\' == \'GAME_tags\' then pcall(function() local function doTag(tag) for _, child'
    GAME.lua = GAME.lua .. ' in ipairs(obj) do if child[2] == \'tags\' then doTag(child[1]) else'
    GAME.lua = GAME.lua .. ' doTo(child[1]) end end end doTag(name) end) else doTo(obj) end end)'
end
...
M['setTransitionRotationNoob'] = function(params)
    ...
    GAME.lua = GAME.lua .. ' end if \'' .. type .. '\' == \'GAME_tags\' then pcall(function() local function'
    GAME.lua = GAME.lua .. ' doTag(tag) for _, child in ipairs(obj) do if child[2] == \'tags\' then doTag(child[1]) else'
    GAME.lua = GAME.lua .. ' doTo(child[1]) end end end doTag(name) end) else doTo(obj) end end)'
end
