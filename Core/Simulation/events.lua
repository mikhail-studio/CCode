...
M['onFun'] = function(nested, params)
    local fun = CALC(params[1], nil, true)

    GAME.lua = GAME.lua .. ' pcall(function() ' .. fun .. ' = function() local varsE, tablesE = {}, {}'
...
M['onFunParams'] = function(nested, params)
    ...
    GAME.lua = GAME.lua .. ' pcall(function() ' .. fun .. ' = function(...) local varsE, tablesE = {}, {} ' .. table .. ' = COPY_TABLE_P({...})'
    M.requestNestedBlock(nested) GAME.lua = GAME.lua .. ' end end)'
end
...
M['onTouchBegan'] = function(nested, params)
    ...
    GAME.lua = GAME.lua .. ' pcall(function() ' .. fun .. ' = function(p) if p.phase == \'began\' then'
    GAME.lua = GAME.lua .. ' local varsE, tablesE, p = {}, {}, COPY_TABLE(p) ' .. table
...
M['onTouchEnded'] = function(nested, params)
    ...
    GAME.lua = GAME.lua .. ' pcall(function() ' .. fun .. ' = function(p) if p.phase == \'ended\''
    GAME.lua = GAME.lua .. ' or p.phase == \'cancelled\' then local varsE, tablesE, p = {}, {}, COPY_TABLE(p) ' .. table
    GAME.lua = GAME.lua .. ' = {name = p.target.name, x = p.x, y = p.y,'
...
M['onTouchMoved'] = function(nested, params)
    ...
    GAME.lua = GAME.lua .. ' pcall(function() ' .. fun .. ' = function(p) if p.phase == \'moved\' then'
    GAME.lua = GAME.lua .. ' local varsE, tablesE, p = {}, {}, COPY_TABLE(p) ' .. table
    GAME.lua = GAME.lua .. ' = {name = p.target.name, x = p.x, y = p.y, xStart = p.xStart, yStart = p.yStart,'
    GAME.lua = GAME.lua .. ' id = p.id, xDelta = p.xDelta, yDelta = p.yDelta}'
    M.requestNestedBlock(nested) GAME.lua = GAME.lua .. ' end end end)'
...
M['onSliderBegan'] = function(nested, params)
    local fun = CALC(params[1], nil, true)
    local var = CALC(params[2], nil, true)
